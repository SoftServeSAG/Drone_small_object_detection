...
def main():
    ...
    input_images = glob.glob(os.path.join(args.input[0], "*.jpg"))
    model_result = dict()
    for it in range(len(input_images)):
        args.input = input_images[it]
        ...
        input_image_name = input_images[it].split('/')[1]
        model_result[input_image_name] = dict()
        model_result[input_image_name]["boxes"] = []
        model_result[input_image_name]["scores"] = []
        input_blob = next(iter(net.inputs))
        n, c, h, w = net.inputs[input_blob].shape
        images = np.ndarray(shape=(n, c, h, w))
        images_hw = []
        for i in range(n):
            image = cv2.imread(input_images[it])
            ih, iw = image.shape[:-1]
        ...
        for number, proposal in enumerate(data):
            if proposal[2] > 0:
                ...
                model_result[input_image_name]["boxes"].append([ymin, xmin, ymin + ymax, xmax + xmin])
                model_result[input_image_name]["scores"].append(proposal[2])
        ...
        for imid in classes:
            tmp_image = cv2.imread(input_images[it])
            for box in boxes[imid]:
                cv2.rectangle(tmp_image, (box[0], box[1]), (box[2], box[3]), (232, 35, 244), 2)
            cv2.imwrite("output/{}".format(input_image_name), tmp_image)
    ...
    with open('data.json', 'w') as outfile:
        json.dump(model_result, outfile, cls=MyEncoder)
